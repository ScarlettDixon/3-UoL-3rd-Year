...
int main(int argc, char **argv) {
	...
	try {
		...
		kernel_add.setArg(2, buffer_C); // removed for averaging kernel
		...
		cerr << kernel_add.getWorkGroupInfo<CL_KERNEL_WORK_GROUP_SIZE>(device) << endl;
		//4.3 Copy the result from device to host
		queue.enqueueReadBuffer(buffer_C, CL_TRUE, 0, vector_size, &C[0]); //removed for averaging
		//queue.enqueueReadBuffer(buffer_B, CL_TRUE, 0, vector_size, &B[0]); //Used for averaging
